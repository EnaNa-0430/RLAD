...
def train(num_LP, num_AL, discount_factor):
    ...
    for j in range(len(percentage)):
        ...
        for i in range(len(dataset_dir)):
            ...
            # environment for testing bug!!!!!!!!!
            # env_test = env
            # env_test.rewardfnc = RNNBinaryRewardFucTest

            # 在 train() 的对应位置，将 env_test = env 改为下面：
            # 创建独立的测试环境（避免引用同一对象）
            env_test = EnvTimeSeriesfromRepo(dataset_dir[i])
            env_test.statefnc = RNNBinaryStateFuc
            env_test.rewardfnc = RNNBinaryRewardFucTest
            env_test.timeseries_curser_init = n_steps
            env_test.datasetfix = DATAFIXED
            env_test.datasetidx = 0
            # 如果 env 有其它重要字段（例如 datasetsize, states_list 等），也手动赋值或确保 EnvTimeSeriesfromRepo 初始化后相同
